// embed.js — self-contained Enigma Syndicate card embed
(async function(){
  // ----- 0) Configuration -----
  ...
  const css = `
    ...
    #demo-errors {
      font-size:.85rem; color:#c00; background:#fee; padding:8px;
  ...
  try {
    ...
    // ----- 5) Authenticate anonymously & capture UID -----
    let uid = null;
    await new Promise((res, rej) => {
    ...
    playerRef.once('value').then(snapshot => {
      ...
      // Real‐time updates:
      playerRef.on('value', snap => renderCard(snap.val()));
    }).catch(e => {
      errBox.textContent = 'DB error: ' + e.message;
    });
    ...
    errBox.textContent = 'Engine error: ' + err.message;
    console.error(err);
  }
})();
